...
btnChart.addEventListener('click', (evt) => {
    ...
    console.log('[DEBUG COMPAT] Button clicked');
    ...
    console.log('[DEBUG COMPAT] Validation:', response);
    ...
    if (response !== true) {
        console.log('[DEBUG COMPAT] Validation failed');
        output2.innerHTML = '';
        wrongDate.innerHTML = response;
        compatibilityContainer.classList.add('display-none');
    } else {
        console.log('[DEBUG COMPAT] Calculating compatibility...');
        ...
        fillMatrix(person, secondPerson);
                ...
                // Создаем объект совместимости для блоков расшифровки
                const compatibility = {
                    points: {
                        apoint: compatibilityMatrix.find(item => item.id === 'compatibilityApoint').value,
                        bpoint: compatibilityMatrix.find(item => item.id === 'compatibilityBpoint').value,
                        cpoint: compatibilityMatrix.find(item => item.id === 'compatibilityCpoint').value,
                        dpoint: compatibilityMatrix.find(item => item.id === 'compatibilityDpoint').value,
                        epoint: compatibilityMatrix.find(item => item.id === 'compatibilityEpoint').value,
                        fpoint: compatibilityMatrix.find(item => item.id === 'compatibilityFpoint').value,
                        gpoint: compatibilityMatrix.find(item => item.id === 'compatibilityGpoint').value,
                        hpoint: compatibilityMatrix.find(item => item.id === 'compatibilityHpoint').value,
                        jpoint: compatibilityMatrix.find(item => item.id === 'compatibilityJpoint').value,
                        mpoint: compatibilityMatrix.find(item => item.id === 'compatibilityMpoint').value,
                        npoint: compatibilityMatrix.find(item => item.id === 'compatibilityNpoint').value,
                        spoint: compatibilityMatrix.find(item => item.id === 'compatibilitySpoint').value,
                        vpoint: compatibilityMatrix.find(item => item.id === 'compatibilityVpoint').value,
                        union: compatibilityMatrix.find(item => item.id === 'compatibilityUnion').value
                    }
                };
